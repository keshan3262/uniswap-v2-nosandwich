...
contract ButterRouter02 is IButterRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = ButterLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = ButterLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'ButterRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = ButterLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'ButterRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidityWithPermit(
        ...
        IButterPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? ButterLibrary.pairFor(factory, output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'ButterRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = ButterLibrary.sortTokens(input, output);
            IButterPair pair = IButterPair(ButterLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = ButterLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
